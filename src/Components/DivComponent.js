...
export default function DivComponent({
  ...
  return (
    <div className="element" style={{ order: order }}>
      ...
      <img src={photoDiv ? photoDiv.results[indexDiv].urls.thumb: console.log('noooon')}/>
      <IconContext.Provider value={{ className: "react-icons" }}>
        {children}
        {console.log('photooooo', photo)}
        {order > 1 && (
          <button onClick={() => handleLeft(indexDiv)} className='button_left'>
            <BsFillArrowLeftCircleFill style={{ stroke: "rgba(233, 230, 233, 0.2)", strokeWidth: "1", borderRadius:"50%"}}/>
          </button>
        )}
        {order < 8 && (
